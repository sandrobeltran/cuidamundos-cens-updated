...
export async function DELETE(req: NextRequest, context: { params: any }) {
    ...
    try {
        ...
        const comment = evidence.comments.find(comment => comment._id === commentId)

        if (comment?.author !== uid) {
            return NextResponse.json(
                { status: "error", message: "No tienes permisos para hacer esta aciión" },
                { status: 403 },
            );
        }

        const commentIndex = evidence.comments.findIndex(comment => comment._id === commentId)

        const commentsCopy = [...evidence.comments]
        commentsCopy.splice(commentIndex, 1)

        const updatedEvidence = await Evidence.findByIdAndUpdate(evidenceId, { comments: commentsCopy }, { new: true })
